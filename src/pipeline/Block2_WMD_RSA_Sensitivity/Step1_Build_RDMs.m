%% Step1_Build_RDMs: Build Patient RDMs for Spatial Sensitivity Analysis
%
% Purpose:
% Construct seven patient-specific Representational Dissimilarity Matrices
% (RDMs) for the TJU and JLH cohorts using the same five primary RDM steps as
% Block2_WMD_RSA, followed by two spatial sensitivity RDMs. The analysis
% excludes the 27 cerebellar regions and each patient's surgery-affected
% regions with at least 5 percent cavity overlap.
%
...
% 6. EUCLIDEAN_DISTANCE_RDM: Euclidean distance between ROI centroids in
%    millimeter MNI coordinates.
% 7. CAVITY_DISTANCE_DIFF_RDM: Absolute pairwise difference in distance from
%    the patient-specific surgical cavity centroid.
%
% Author: Qirui Zhang
...
surgery_mask_suffixes = [".nii.gz"; "_POST1_Run1_ROI.nii.gz"];

%% Atlas definition and fixed interregional Euclidean distances
...
% Atlas centroids are calculated once in millimeter MNI coordinates.
atlas_nii_path = fullfile( ...
    project_root, 'Data', 'atlas', 'cocoyeo143_CAT12_GMV_grid.nii');
atlas_info = niftiinfo(atlas_nii_path);
atlas_volume = round(double(niftiread(atlas_info)));
n_atlas_rois = numel(full_atlas_roi_ids);
atlas_centroids = zeros(n_atlas_rois, 3);
for atlas_roi_index = 1:n_atlas_rois
    roi_id = full_atlas_roi_ids(atlas_roi_index);
    roi_voxel_indices = find(atlas_volume == roi_id);
    [voxel_x, voxel_y, voxel_z] = ind2sub( ...
        size(atlas_volume), roi_voxel_indices);
    mean_voxel_coordinate = [ ...
        mean(voxel_x), mean(voxel_y), mean(voxel_z), 1];
    mean_mni_coordinate = mean_voxel_coordinate * atlas_info.Transform.T;
    atlas_centroids(atlas_roi_index, :) = mean_mni_coordinate(1:3);
end

% The fixed 143-region Euclidean matrix is subsetted for each patient below.
full_euclidean_matrix = zeros(n_atlas_rois, n_atlas_rois);
for first_atlas_roi = 1:n_atlas_rois
    for second_atlas_roi = 1:n_atlas_rois
        if first_atlas_roi ~= second_atlas_roi
            full_euclidean_matrix(first_atlas_roi, second_atlas_roi) = norm( ...
                atlas_centroids(first_atlas_roi, :) - ...
                atlas_centroids(second_atlas_roi, :));
        end
    end
end
noncerebellar_euclidean = full_euclidean_matrix( ...
    noncerebellar_mask, noncerebellar_mask);
noncerebellar_centroids = atlas_centroids(noncerebellar_mask, :);
...
for dataset_index = 1:numel(dataset_names)
    ...
    output_root = fullfile( ...
        project_root, 'results', 'Block2_WMD_RSA_Sensitivity', dataset);
    ...
    for subject_index = 1:numel(subject_ids)
        ...
        %% Construct the two spatial sensitivity RDMs

        % Euclidean distances use the same retained ROI order as the primary RDMs.
        euclidean_rdm = noncerebellar_euclidean( ...
            retained_roi_indices, retained_roi_indices);
        euclidean_rdm(1:n_rois + 1:end) = 0;

        % The fixed 1 mm cavity mask supplies the patient-specific cavity centroid.
        surgery_mask_path = fullfile( ...
            project_root, 'Data', dataset, 'surgery_mask', 'Orig_1mm', ...
            subject_id + surgery_mask_suffixes(dataset_index));
        surgery_mask_info = niftiinfo(surgery_mask_path);
        surgery_mask = double(niftiread(surgery_mask_info)) > 0;
        cavity_voxel_indices = find(surgery_mask);
        [cavity_x, cavity_y, cavity_z] = ind2sub( ...
            size(surgery_mask), cavity_voxel_indices);
        mean_cavity_voxel = [ ...
            mean(cavity_x), mean(cavity_y), mean(cavity_z), 1];
        mean_cavity_mni = mean_cavity_voxel * surgery_mask_info.Transform.T;
        cavity_centroid = mean_cavity_mni(1:3);

        retained_centroids = noncerebellar_centroids( ...
            retained_roi_indices, :);
        distance_to_cavity = sqrt(sum( ...
            (retained_centroids - cavity_centroid) .^ 2, 2));
        cavity_diff_rdm = abs(distance_to_cavity - distance_to_cavity');
        cavity_diff_rdm(1:n_rois + 1:end) = 0;

        matrices = {pre_fc_rdm, post_fc_rdm, func_reorg_rdm, ...
            morph_reorg_rdm, wmd_rdm, euclidean_rdm, cavity_diff_rdm};
        matrix_names = ["PRE_FC_RDM", "POST_FC_RDM", ...
            "FUNC_REORG_RDM", "MORPH_REORG_RDM", "WMD_RDM", ...
            "EUCLIDEAN_DISTANCE_RDM", "CAVITY_DISTANCE_DIFF_RDM"];
